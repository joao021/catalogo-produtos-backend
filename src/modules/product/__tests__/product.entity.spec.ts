...
describe('Product Entity', () => {
  it('should create a product entity', () => {
    ...
    expect(product.price12Months).toBe(1999.8);
    expect(product.price6Months).toBe(1799.82);
    expect(product.price3Months).toBe(999.9);
  ...
  it('should set the properties correctly', () => {
    ...
    expect(product.price6Months).toBe(1799.82);
    expect(product.price3Months).toBe(999.9);
    expect(product.imageUrlFront).toBe(
      'http://example.com/another-image-front.jpg',
    );
    expect(product.imageUrlSide).toBe(
      'http://example.com/another-image-side.jpg',
    );
    expect(product.imageUrlBack).toBe(
      'http://example.com/another-image-back.jpg',
    );
  });
});
